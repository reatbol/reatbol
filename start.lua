...
print("::reatbol::")
local RunBot = io.open("reatbol", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/Losfer
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr DLASH.lua
wget "https://raw.githubusercontent.com/TEAMREATBOL/reatbol/master/reatbol.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./reatbol.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S reatbol -X kill
screen -S reatbol ./reatbol
done
]])
RunTs:close()
end
